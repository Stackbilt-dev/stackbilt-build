...
function writeCachedBuild(intention, files, configPath) {
    const dir = configPath || '.charter';
    if (!fs.existsSync(dir))
        fs.mkdirSync(dir, { recursive: true });
    const core = buildScaffold(intention);
    fs.writeFileSync(path.join(dir, 'last-build.json'), JSON.stringify({
        intention,
...
function localScaffoldToResult(intention) {
    const core = buildScaffold(intention);
    const roleMap = {
        entry: 'scaffold',
        config: 'config',
...
async function persistToPlatform(intention, oracle, apiKey) {
    try {
...
function phaseDetail(label, result) {
...
export async function runCommand(options, args) {
    ...
    if (!description)
        throw new CLIError('Empty description.');
    ...
    if (options.format === 'json') {
        ...
        const output = { ...result, outputDir: resolvedOutput, dryRun };
        if (!dryRun) {
    ...
    if (persist && resolved?.apiKey && !dryRun) {
        ...
        if (persisted) {
            ...
            if (oracle)
                console.log('  → Oracle polish queued');
        }
    }
    console.log('');
    return EXIT_CODE.SUCCESS;
