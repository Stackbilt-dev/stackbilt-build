...
export async function scaffoldCommand(options, args) {
    ...
    if (fs.existsSync(cachePath)) {
        ...
        if (cached.files && cached.files.length > 0) {
            ...
            resolvedFiles = cached.files.map(f => [f.path, f.content]);
        }
            ...
            resolvedFiles = generated.map(f => [f.path, f.content]);
        }
        else if (cached.scaffold && Object.keys(cached.scaffold).length > 0) {
            // Legacy BuildResult shape
            resolvedFiles = Object.entries(cached.scaffold);
        }
        else {
    ...
    if (!intention) {
        throw new CLIError('No cached build found and no intention provided.\n' +
...
function writeScaffoldFiles(outputDir, files) {
