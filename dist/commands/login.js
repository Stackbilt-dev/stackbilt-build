...
import { loadCredentials, saveCredentials, clearCredentials, API_KEY_ENV_VAR, } from '../credentials.js';
import { EngineClient } from '../http-client.js';
export async function loginCommand(options, args) {
    ...
    if (!key) {
        ...
        if (existing) {
            ...
            if (existing.baseUrl)
                console.log(`Engine: ${existing.baseUrl}`);
        }
        else {
    ...
    try {
        ...
        throw new CLIError(`Could not reach engine: ${err.message}`);
    }
}
