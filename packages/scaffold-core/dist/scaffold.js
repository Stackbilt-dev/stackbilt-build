...
// Template helpers
// ============================================================================
function workerName(pattern) {
    return pattern.replace(/-/g, '_');
}
function bindingToml(bindings) {
    const lines = [];
...
function buildEnvInterface(bindings) {
    const fields = [];
    if (bindings.includes('d1'))
        fields.push('  DB: D1Database;');
    if (bindings.includes('kv'))
        fields.push('  KV: KVNamespace;');
    if (bindings.includes('r2'))
        fields.push('  BUCKET: R2Bucket;');
    if (bindings.includes('queues'))
        fields.push('  QUEUE: Queue;');
    if (bindings.includes('do'))
        fields.push('  DO: DurableObjectNamespace;');
    if (bindings.includes('ai'))
        fields.push('  AI: Ai;');
    return fields.join('\n') || '  // No bindings detected';
}
function buildMainHandler(pattern, bindings) {
 ...
 */
export function buildScaffold(intention, result) {
