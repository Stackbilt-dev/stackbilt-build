...
function detectPattern(intention) {
    let best = 'rest-api';
    let bestScore = 0;
    for (const rule of PATTERN_RULES) {
        let score = 0;
        for (const { terms, weight } of rule.signals) {
            if (terms.test(intention))
                score += weight;
        }
        if (score > bestScore) {
            bestScore = score;
...
function detectConfidence(score, intention) {
    const wordCount = intention.trim().split(/\s+/).length;
    if (score >= 3)
        return 'high';
    if (score >= 2 && wordCount >= 3)
        return 'medium';
    return 'low';
}
function detectVerification(intention) {
    if (/\b(jwt|json.?web.?token)\b/i.test(intention))
        return 'jwt-auth';
    if (/\b(oauth|oauth2)\b/i.test(intention))
        return 'oauth';
    if (/\b(hmac|webhook.?secret)\b/i.test(intention))
        return 'hmac';
    if (/\b(ed25519|signature)\b/i.test(intention))
        return 'ed25519';
    if (/\b(api.?key|bearer)\b/i.test(intention))
        return 'api-key';
    if (/\b(auth|login|session|token|secure)\b/i.test(intention))
        return 'jwt-auth';
    return 'none';
}
function detectRouteShape(pattern, intention) {
    if (pattern === 'queue-consumer')
        return 'event';
    if (pattern === 'cron-worker')
        return 'event';
    if (/\b(websocket|stream|realtime|live)\b/i.test(intention))
        return 'stream';
    if (/\b(rpc|procedure|call)\b/i.test(intention))
        return 'rpc';
    if (/\b(webhook|event|hook|push)\b/i.test(intention))
        return 'event';
    return 'rest';
}
function detectDispatch(pattern) {
    if (pattern === 'queue-consumer')
        return 'queue-consumer';
    if (pattern === 'cron-worker')
        return 'cron';
    if (pattern === 'discord-bot' || pattern === 'stripe-webhook' || pattern === 'github-webhook')
        return 'event-handler';
    return 'resource-router';
}
function detectBindings(intention) {
    const bindings = new Set();
    if (/\b(d1|sql|database|sqlite|table|migration|schema|entity|record)\b/i.test(intention))
        bindings.add('d1');
    if (/\b(kv|cache|session|fast.?read|key.?value|config)\b/i.test(intention))
        bindings.add('kv');
    if (/\b(r2|storage|file|image|video|upload|download|attachment|asset|bucket)\b/i.test(intention))
        bindings.add('r2');
    if (/\b(queue|job|background|async.?process|worker.?queue)\b/i.test(intention))
        bindings.add('queues');
    if (/\b(durable.?objects?|realtime|websocket|live|collaborative)\b/i.test(intention))
        bindings.add('do');
    if (/\b(ai|llm|inference|embeddings?|vector|model|openai|anthropic|cerebras)\b/i.test(intention))
        bindings.add('ai');
    return Array.from(bindings);
}
function detectQualityProfile(intention) {
    const lower = intention.toLowerCase();
    return QUALITY_TERMS.filter(t => lower.includes(t.toLowerCase()));
}
function detectTier(bindings, qualityProfile, confidence) {
    const complexity = bindings.length + qualityProfile.length;
    if (complexity >= 5 || bindings.length >= 4)
        return 3;
    if (complexity >= 2 || confidence === 'high')
        return 2;
    return 1;
}
// ============================================================================
 ...
 */
export function classify(intention) {
